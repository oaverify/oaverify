...
7. Grep for `.source` on a `compileSchema` result, and for
   `const { source } = compileSchema(...)`, which a search for the
   property access alone misses. It is empty unless the compile passes
   `retainSource: true`. A snapshot assertion that reads it without
   asking now passes against an empty string rather than failing.
