...
| `unknownFormats`            | A `format` nothing registered covers: `"ignore"` (default) leaves it asserting nothing, `"error"` refuses to compile. The built-ins count, so `formats: {}` refuses nothing. Compilation is lazy, so an untouched operation surfaces on its first request. |
| `requireResponseBody`       | Treat a declared response body with neither `res.body` nor `res.bodyPresent` as an error, except for HEAD and bodyless statuses. Default `false`; OpenAPI takes no position, so the rule is opt-in.                                                        |
