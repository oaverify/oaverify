...
The behavior deltas surfaced by real eov-to-oaverify migrations. Most are
defensible improvements; a few are cosmetic. Either way, expect
fixture / test churn.

| What                            | eov                                                                                            | oaverify                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Empty POST, no Content-Type** | 415 unsupported-media-type                                                                     | Decided by the body parser, not by the adapter. Express 4 + `express.json()`: **415**, same as eov. Express 5 + `express.json()`, or either major with no parser: **400** body-required. Express 4's parser leaves `req.body` as `{}` when it declines to parse, so a body looks present and the absent `Content-Type` is answered first; Express 5's leaves it `undefined`. |
| **Undeclared query parameters** | 400 `Unknown query parameter 'x'` by default; opt out with `allowUnknownQueryParameters: true` | Accepted by default. Set `createValidator(spec, { strictQueryParameters: true })` to reject them with a `query-param` error. **The one delta here that is silent**: nothing fails on migration day, the check is just gone.                                                                                                                                                  |
...
The **Empty POST** and **Undeclared query parameters** rows were
measured on 2026-08-23 against eov 5.6.2, express 4.22.2 and express
5.2.1, and the two empty-POST answers are pinned by
`framework-tests/test/oav-express4.test.ts` and its express5 sibling.
The rest of the table came from real migrations and carries no version
stamp, so check a row against your own eov version before relying on
it.

For the `oneOf [array<binary>, binary]` pattern specifically: the
spec was already ambiguous before oaverify surfaced it (the `format:
...
| eov option                              | oaverify equivalent                                                                                             |
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
...
| eov option                                     | oaverify equivalent                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `validateRequests: true`                       | `validator.validateRequest(...)` in your middleware, or `validateRequests(validator)` from `@oaverify/express4`.                                                                                                                                                                                      |
| `validateRequests.allErrors`                   | `maxErrors: Number.POSITIVE_INFINITY`. oaverify defaults to `maxErrors: 1` (fast-fail); raise it to collect more leaves.                                                                                                                                                                              |
...
| `validateRequests.allowUnknownQueryParameters` | Inverted, and defaulted the other way: oaverify accepts unknown query parameters unless you pass `createValidator(spec, { strictQueryParameters: true })`. eov rejects them unless you opt out, so a migration that does not set this loses the check silently. See [strictness.md](./strictness.md). |
...
| eov option                  | oaverify equivalent                                                                                                                                                                                                                                                                              |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `validateSecurity: true`    | Off by default in oaverify (real apps gate security upstream of validation). Opt in with `createValidator(spec, { validateSecurity: "shape" })` for shape-only checks on `bearer` / `basic` / `apiKey`, or `"strict"` to fail on schemes the validator can't shape-check (oauth2 / oidc / mTLS). |
| `validateSecurity.handlers` | Your own auth middleware, run before the validator. `oaverify` only checks credential **shape**, not validity. For declarative per-scheme dispatch, see the [integration.md security recipe](./integration.md#per-scheme-auth-dispatch).                                                         |
...
| eov option          | oaverify equivalent                                                                 |
| ------------------- | ----------------------------------------------------------------------------------- |
| `operationHandlers` | Your own Express routes. `oaverify` doesn't auto-load handlers from the filesystem. |
...
- **Smaller install footprint.** `oaverify` depends on `yaml`,
...
- **No mutation of `req`.** eov attaches `req.openapi`, coerces
  types, and replaces `req.body` after deserialise. `oaverify` reads its
...
`oaverify` expects a plain string predicate:
...
`oaverify`'s `format` keyword only applies to string values (per JSON
...
`oaverify` defaults to the same first-leaf summary. For an all-issues
...
// All leaves, one per line. oaverify's default all-issues shape.
...
The leaves `oaverify`'s validator emits for HTTP-level failures (missing
...
parent. `oaverify` keeps the leaf path on the field (it's the
...
One delta from `eov` remains. `oaverify` uses dotted paths
