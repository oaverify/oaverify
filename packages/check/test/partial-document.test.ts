...
describe("the cost of withholding by code alone", () => {
  /**
   * The classification above is per code. The cheaper rule is "a hole
   * exists anywhere, so withhold every absence-based code"; the dearer
   * one is "withhold a finding when a hole lies inside the subtree its
   * verdict read". This fixture is what separates them: one operation
   * has a genuine `path-param-undeclared` that no missing file could
   * supply, and a different operation has the hole.
   *
   * The per-code rule withholds the genuine finding. The number below
   * is what that costs on a document with one hole and one true
   * absence-based finding, which is the smallest case where the two
   * rules disagree at all.
   */
  const unrelated = map([
    [
      "main.json",
      {
        ...
        info,
        paths: {
          // Genuine: `{id}` is declared by nobody, and the operation
          // holds no reference at all, so nothing that could arrive
          // would declare it.
          "/a/{id}": { get: { responses: { "200": okResponse } } },
          // The hole, in a different operation.
          "/b": { get: { responses: { "200": { $ref: "ext.json#/Ok" } } } },
        },
      },
    ],
    ["ext.json", { Ok: okResponse }],
  ...
  it("keeps a true absence-based finding that the hole has nothing to do with", async () => {
    const holed = await findingsFor(unrelated, "ext.json");
    const undeclared = holed.filter((f) => f.code === "path-param-undeclared");
    expect(undeclared).toHaveLength(1);
    expect(undeclared[0]!.target?.pointer).toBe("/paths/~1a~1{id}/get");
    // Not an artefact: the whole run reports it too.
    expect(await artefactCodes(unrelated)).not.toContain("path-param-undeclared");
  });

  it("counts what the per-code rule would drop here", async () => {
    const holed = await findingsFor(unrelated, "ext.json");
    // Absence-based codes, from the classification the suites above
    // measured. A rule keyed on code alone withholds every finding
    // carrying one of these as soon as any hole exists.
    const absenceBased = new Set([
      "unused-component",
      "unused-tag",
      "unreachable-defs",
      "path-param-undeclared",
    ]);
    const dropped = holed.filter((f) => absenceBased.has(f.code));
    const artefacts = await artefactCodes(unrelated);
    // One true finding lost, no artefact prevented: on this document the
    // per-code rule is all cost and no benefit.
