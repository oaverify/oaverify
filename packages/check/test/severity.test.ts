...
describe("input the flag refuses rather than half-applies", () => {
  ...
  it("refuses a code no check emits, naming the family's real members", () => {
    ...
    expect(() => parse("unsatisfiable/patern-length=error")).toThrow(
      /"unsatisfiable\/" holds unsatisfiable\/composed-enum-empty, unsatisfiable\/composed-enum-members, unsatisfiable\/composed-properties, unsatisfiable\/enum-member-type, unsatisfiable\/pattern-length/,
    );
  });
