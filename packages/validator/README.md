...
| `unknownFormats`            | A `format` nothing registered covers: `"ignore"` (default) asserts nothing, `"error"` refuses to compile. The built-ins count, so `formats: {}` refuses nothing. |
