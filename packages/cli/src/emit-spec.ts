...
export interface EmitSpecOptions {
  ...
  /**
   * Reject requests that do not satisfy the declared
   * {@link OperationObject.security}, or the document-level
   * `security` when the operation declares none. Default `"off"`,
   * matching `ValidatorOptions.validateSecurity`, whose TSDoc carries
   * the contract: the check confirms a credential of the declared kind
   * arrived, and never that it is valid.
   *
   * `"off"` emits no security machinery at all, so a module compiled
   * with the default is smaller than one compiled before this option
   * existed, which always checked operation-level requirements and
   * never document-level ones.
   *
   * A caller wanting real enforcement composes around the validator
   * rather than configuring it: `getOperation` returns the matched
   * operation with its requirement and scopes, which is more than this
   * check ever inspects. See the CLI README.
   */
...
export function emitSpec(document: OpenAPIDocument, options: EmitSpecOptions = {}): string {
  ...
  const securityMode = options.validateSecurity ?? "off";
  const checksSecurity = securityMode !== "off";
  ...
  const securityWarning = uninspectableSecurityWarning(document, securityMode, resolveRef);
  if (securityWarning !== undefined) warnings.push(securityWarning);
  ...
  return [
    ...
    ...(checksSecurity
      ? [
          ...
          "//",
          "// Called for every operation, not only those declaring their own",
          "// `security`. `compileOperationSecurity` owns the precedence",
          "// (`operation.security ?? document.security`) and answers",
          "// `undefined` when neither declares anything, so testing",
          "// `op.__security` here skipped every operation inheriting from the",
          "// document and dropped document-level security entirely (#895).",
    ...
    renderValidateRequestTree(returnValues, usesContentParameters, checksSecurity),
    "",
...
interface BuildEmittedOpArgs {
  pathPattern: string;
  method: string;
  /** Whether `validateSecurity` is on; see {@link EmitSpecOptions.validateSecurity}. */
    ...
    JSON.stringify(
      {
        ...
        // Read only by the load-time fold, which is emitted only when
        // the mode is on. Carrying it otherwise puts a document's
        // security requirements into a module that never looks at them.
        ...(args.checksSecurity ? { __security: security } : {}),
      },
      null,
      2,
    ),
...
/**
 * Warn when a security mode is on and nothing it can inspect is
 * required.
 *
 * `"shape"` silently satisfies schemes the validator cannot look at
 * (`oauth2`, `openIdConnect`, `mutualTLS`, HTTP schemes other than
 * bearer and basic), which is the documented behaviour and is what
 * keeps it from producing spurious 401s. On a document whose every
 * requirement names such a scheme, the consequence is that the gate
 * passes every request, including one carrying no credential at all.
 * That reads as a check and is not one.
 *
 * Both the mode and every scheme are known here, so the artifact can
 * say so rather than leaving it to be discovered. Requirements are what
 * is inspected, not `components.securitySchemes`: a scheme nobody
 * requires says nothing about what the gate will do.
 *
 * `"strict"` needs no warning. It rejects an uninspectable scheme
 * instead of passing it, which is the whole of its difference.
 */
function uninspectableSecurityWarning(
  document: OpenAPIDocument,
  mode: "off" | "shape" | "strict",
  resolveRef: <T>(v: T | ReferenceObject | undefined) => T | undefined,
): string | undefined {
  if (mode !== "shape") return undefined;
  const required = new Set<string>();
  const collect = (reqs: unknown): void => {
    if (!Array.isArray(reqs)) return;
    for (const r of reqs) {
      if (r === null || typeof r !== "object") continue;
      for (const name of Object.keys(r as Record<string, unknown>)) required.add(name);
    }
  };
  ...
  for (const item of Object.values(document.paths ?? {})) {
    ...
    for (const [key, op] of Object.entries(item as Record<string, unknown>)) {
      if (key === "parameters" || op === null || typeof op !== "object") continue;
      collect((op as { security?: unknown }).security);
    }
  }
  ...
  for (const name of [...required].sort()) {
    const raw = (schemes as Record<string, unknown>)[name];
    const scheme = resolveRef<SecuritySchemeObject>(raw as SecuritySchemeObject | undefined);
    const type = scheme?.type;
    const http = (scheme as { scheme?: string } | undefined)?.scheme?.toLowerCase();
    const inspectable =
      type === "apiKey" || (type === "http" && (http === "bearer" || http === "basic"));
    if (!inspectable) opaque.push(`${name} (${type ?? "undeclared"})`);
  }
  if (opaque.length !== required.size) return undefined;
  return (
    `validateSecurity "shape" cannot inspect any scheme this document requires ` +
    `(${opaque.join(", ")}), so the emitted gate accepts every request, including one ` +
    `carrying no credential. Use "strict" to refuse them instead, or verify credentials ` +
...
function renderValidateRequestTree(
  returnValues: boolean,
  contentParameters: boolean,
  checksSecurity: boolean,
): string {
  ...
  // Emitted only when `validateSecurity` is on. With it off there is no
  // `compiledSecurity` on any op, so the test would be dead weight in
  // every module, and the runtime's default is off too.
  const securityGate = checksSecurity
    ? `  // Security gate.
