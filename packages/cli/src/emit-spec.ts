...
function buildEmittedOp(args: BuildEmittedOpArgs): EmittedOp {
  ...
  const hasHeaderParameters = [...combined.values()].some((p) => p.in === "header");
  ...
  return {
    pathPattern,
    method,
    usesHeaderHelper: hasHeaderParameters || hasResponseHeaders,
    stateLiteral,
    ...
    if (resp !== undefined) {
      ...
      if (res.headers !== undefined) {
        for (const [name, hdr] of Object.entries(resp.headers)) {
          const raw = __readHeader(res.headers, name);
