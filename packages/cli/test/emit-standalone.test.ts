...
describe("emitStandalone", () => {
  ...
  it("marks the emitted source as generated", () => {
    const source = emitStandalone({ type: "integer" } as SchemaOrBoolean, { dialect: "2020-12" });
    expect(source).toMatch(/^\/\/ Generated by .*Do not edit by hand\./);
  });

  // Every import the emitted module makes has to resolve against the
  // caller's chosen prefix, otherwise the module is unloadable wherever
  // the internal package layout isn't on disk. Asserted over the
  // extracted specifiers rather than over literal import lines, so
  // regrouping the imports doesn't fail the test.
  it.each([
    ["the default prefix", undefined, "@oaverify/core"],
    ["a custom prefix", "my-bundled-oav", "my-bundled-oav"],
  ])("imports only from %s", (_label, importPrefix, expected) => {
    const source = emitStandalone({ type: "integer" } as SchemaOrBoolean, {
      dialect: "2020-12",
      ...(importPrefix === undefined ? {} : { importPrefix }),
    });
    const specifiers = [...source.matchAll(/^import .*? from "([^"]+)";$/gm)].map((m) => m[1]!);
    expect(specifiers.length).toBeGreaterThan(0);
    for (const s of specifiers) {
      expect(s === expected || s.startsWith(`${expected}/`)).toBe(true);
    }
  });
});
