...
oaverify check <spec> --skip format-not-validated                        # drop a code, and say so
...
| `--only <classes>`                            | check                                            | Comma-separated subset of `conformance`, `hygiene`, `schema`, `examples`, `redos`. Default: all.                                                                                                                                                                                                                                                                                                                                                   |
| `--findings <terms>`                          | check                                            | One flag over `--only` and `--skip`: comma-separated terms in `--severity`'s key grammar, `-` prefixed to exclude. Terms without `-` also decide which checks run, so they are how a run avoids work; terms with `-` drop findings the checks produced. Order is irrelevant, `malformed` is refused, and passing it beside `--only` or `--skip` is a usage error. See [docs/strictness.md](../../docs/strictness.md#one-flag-for-both---findings). |
...
| `--skip <keys...>`                            | check                                            | Drop findings before anything reads them: comma-separated keys in `--severity`'s grammar. What was dropped is reported in every format; `malformed` is refused. See [docs/strictness.md](../../docs/strictness.md#when-you-do-not-want-the-finding-at-all).                                                                                                                                                                                        |
...
| `--format text\|json`                         | check / stream-check                             | check: one finding per line, or `{ findings }` plus `skipped` when `--skip` dropped anything. stream-check: per-operation table, or the `SpecBudget`.                                                                                                                                                                                                                                                                                              |
